...
export default function PageFormPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => api.get(`/api/cms/pages/${id}`).then(r => r.data.data as Page),
    enabled: isEdit,
  })
  ...
  useEffect(() => {
    ...
    draftTimer.current = setTimeout(() => {
      saveDraft(draftKey, {
        title, slug, content, meta_title: metaTitle,
  ...
  }, [isDirty, isEdit, draftKey, title, slug, content, metaTitle, metaDescription, ogImage, featuredImageId, featuredImageUrl])
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { clearDraft(draftKey); qc.invalidateQueries({ queryKey: ['CMS', 'pages'] }); navigate('/cms/pages') },
    onError: (err: any) => setError(getApiError(err)),
  })

  const updateMutation = useMutation({
    mutationFn: (body: any) => api.put(`/api/cms/pages/${id}`, body),
    onSuccess: () => { clearDraft(draftKey); qc.invalidateQueries({ queryKey: ['CMS', 'pages'] }); navigate('/cms/pages') },
    onError: (err: any) => setError(getApiError(err)),
  })
  ...
  if (isEdit && isLoading) {
    return (
      <main className="content-canvas animate-fade-in">
        <PageHeader title="Loading..." description="Fetching page data" />
        <Card testId="page-form-loading-card"><div className="p-8 text-center text-[var(--on-surface-variant)]">Loading...</div></Card>
      </main>
    )
  }
  ...
  return (
    <main className="content-canvas animate-fade-in">
      <PageHeader
        ...
        description={isEdit ? `Editing: ${title}` : 'Create a new website page'}
        action={
          <Button variant="ghost" icon="arrow_back" onClick={() => navigate('/cms/pages')} data-testid="page-form-back-button">
            Back to Pages
          </Button>
        }
      />
      ...
      {draftBanner.show && (
        <div className="mb-4 p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-sm flex items-center gap-2" data-testid="page-form-draft-banner">
          <span className="material-symbols-outlined text-base">description</span>
          <span className="flex-1">
            You have an unsaved draft from {new Date(draftBanner.savedAt).toLocaleTimeString()}.
          </span>
          <button onClick={restoreDraft} className="text-sm font-medium underline hover:no-underline" data-testid="page-form-draft-restore">Restore</button>
          <button onClick={discardDraft} className="text-sm font-medium underline hover:no-underline" data-testid="page-form-draft-discard">Discard</button>
        </div>
      )}

      {error && (
        <div className="mb-4 p-3 rounded-lg bg-[var(--danger-bg)] text-[var(--danger-text)] text-sm flex items-center gap-2" data-testid="page-form-error">
          <span className="material-symbols-outlined text-base">error</span>
          <span className="flex-1">{error}</span>
          <button onClick={() => setError(null)} className="text-[var(--danger-text)] hover:opacity-70" data-testid="page-form-error-dismiss">
            <span className="material-symbols-outlined text-base">close</span>
          </button>
        </div>
      )}

      <Card testId="page-form-card" padding={false}>
        <div className="p-6 space-y-6">
          <Input label="Title *" value={title} onChange={e => handleTitleChange(e.target.value)} required data-testid="page-form-title" />
          <div>
            <Input
              label="Slug"
              value={slug}
            ...
            />
            {slugConflict && (
              <p className="mt-1 text-xs text-amber-600 flex items-center gap-1" data-testid="page-form-slug-warning">
          ...
          <div>
            ...
            <RichTextEditor content={content} onChange={v => { setContent(v); markDirty() }} testId="page-form-content-editor" />
          </div>
          ...
          <div className="border-t border-[var(--outline-variant)] pt-6">
            ...
            <div className="space-y-3">
              <Input label="Meta Title" value={metaTitle} onChange={e => { setMetaTitle(e.target.value); markDirty() }} data-testid="page-form-meta-title" />
              <div>
          ...
          <div className="border-t border-[var(--outline-variant)] pt-6">
            ...
            {featuredImageUrl && (
              <div className="mt-2">
                <img src={featuredImageUrl} alt="Preview" className="h-24 w-auto rounded border border-[var(--outline-variant)] object-cover" data-testid="page-form-image-preview" />
              </div>
            )}
          </div>
        </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        <span className="text-xs text-[var(--on-surface-variant)]" data-testid="page-form-draft-indicator">
          {isDirty && !saving ? 'Unsaved changes' : ''}
        </span>
        <div className="flex gap-2">
          <Button variant="ghost" onClick={() => navigate('/cms/pages')} data-testid="page-form-cancel">Cancel</Button>
          <Button onClick={handleSubmit} loading={saving} disabled={!title} data-testid="page-form-submit">
